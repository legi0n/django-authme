...
class TestLoginRequiredMixin(TestCase):
    def test_anonymous_user(self):
        ...
        self.assertHttpRedirect(
            self.get_view_response(LoginRequiredView, request)
        )

    def test_authenticated_user(self):
        self.assertHttpOK(
            self.get_view_response(LoginRequiredView)
        )
...
class TestAnonymousRequiredMixin(TestCase):
    def test_anonymous_user(self):
        request = self.build_request(user='anonymous')
        self.assertHttpOK(
            self.get_view_response(AnonymousRequiredView, request)
        )

    def test_authenticated_user(self):
        self.assertHttpRedirect(
            self.get_view_response(AnonymousRequiredView)
        )
...
class TestStaffUserRequiredMixin(TestCase):
    def test_anonymous_user(self):
        request = self.build_request(user='anonymous')
        self.assertHttpRedirect(
            self.get_view_response(StaffUserRequiredView, request)
        )
...
class TestUserPassesTestMixin(TestCase):
    def test_user_pass(self):
        request = self.build_request(
            user_kwargs={'email': 'user@mydomain.com'}
        )
        self.assertHttpOK(
    ...
    def test_user_not_pass(self):
        request = self.build_request(
            user_kwargs={'email': 'user@notmydomain.com'}
        )
        with self.assertRaises(PermissionDenied):
            self.get_view_response(UserPassesTestView, request)
